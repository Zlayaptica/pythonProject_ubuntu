...
def deploy():
    res = []
    upload_files(data['host'], data['login'], data['passwd'], "/home/user/p7zip-full.deb", "/home/user1/p7zip-full.deb")
    res.append(ssh_checkout(data['host'], data['login'], data['passwd'],
                            "echo '111' | sudo -S dpkg -i /home/user1/p7zip-full.deb",
                            'Настраивается пакет'))
    res.append(ssh_checkout(data['host'], data['login'], data['passwd'], "echo '111' | sudo -S dpkg -s p7zip-full",
...
def deploy_apt():
    res = [ssh_checkout(data['host'], data['login'], data['passwd'],
                        ...
                        'Настраивается пакет'), ssh_checkout(data['host'], data['login'], data['passwd'],
                                                             "echo '111' | sudo -S apt list libarchive-zip-perl",
                                                             'установлен')]
